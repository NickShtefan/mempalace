...
class MempalaceConfig:
    ...
    def _normalize_embedding_model(value) -> str:
        ...
        The built-ins (``minilm`` / ``embeddinggemma``) are matched
        case-insensitively so ``MiniLM`` still selects the right class.
        Other values may be local checkpoint paths, which are
        case-sensitive on Linux filesystems — lowercasing them would break
        the lookup. (HuggingFace repo ids resolve case-insensitively
        either way; identity comparisons go through
        ``embedding.current_model_name``, which lowercases consistently.)
        """
        normalized = str(value).strip()
        if normalized.lower() in ("minilm", "embeddinggemma"):
    ...
    def embedding_model(self):
        ...
        new installs since onboarding writes the choice), or any
